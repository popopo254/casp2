...
`
const TS_Get_P1 = (data,callback) =>{
    const { project_id,project_type,project_user } = data

    //console.log('Fetching P1 data with:', { project_id, project_user, project_type });
    ...
    db.query(getP1,[project_id,project_user,project_type],(err,result)=>{
        ...
        //try
         if (result.length === 0) {
            //console.log('No P1 data found for project_id:', project_id);
            return callback(null, []); // Return empty array if no data found
        }
        //
        //console.log("TS_Get_P1 :",result)
        callback(null,result)
    })
}

module.exports = TS_Get_P1