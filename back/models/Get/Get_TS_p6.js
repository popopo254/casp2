...
const getP6 = `
    SELECT 
    projects.project_id, projects.project_type ,projects.project_data_ref, project_data.project_data_p_6
FROM projects 
...
`
const TS_Get_P6 = (data,callback) =>{
    const { project_id,project_type,project_user } = data

    //console.log(project_id,project_user,project_type)
    
    db.query(getP6,[project_id,project_user,project_type],(err,result)=>{
        if(err){
            return callback(err,null)
        }
        //console.log("TS_Get_P6 :",result)
        callback(null,result)
    })
}

module.exports = TS_Get_P6